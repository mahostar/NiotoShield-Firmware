...
class FaceSecurity:
    ...
    def add_notification(self, message):
        """Add a notification to the Supabase notifications table"""
        try:
            ...
            # Remove any emoji replacement code since we're using clean text from the start
            headers = {
                "apikey": self.supabase_key,
                "Authorization": f"Bearer {self.supabase_key}",
        ...
        try:
            ...
            try:
                ...
                if not is_real_person:
                    # Spoof detected - send alert and stop
                    logger.warning("SECURITY ALERT: Spoofing attempt detected!")
                    self.add_notification(NOTIFICATION_TYPES["SPOOF_DETECTED"])
